...
['Nick Mesken','Mario Grundza','Robert Botos','Boguslaw Nowakowski','René Kortekaas','Jan de Vries','Piet Bakker','Kees Jansen'].forEach((n,i)=>add('medewerkers','medewerker',{naam:n,functie:'',actief:true,sort:i+1}));
  ...
  try{
    switch(body.action){
      ...
      case 'projecttarief_save': { // projectafspraken: prijs per tarief voor dit project (wizard)
        ...
        break; }
      case 'medewerker_save': { // teamleider mag namen toevoegen aan de namenwolk
        if('naam' in fields&&!String(fields.naam).trim()) throw new Error('naam leeg'); const naam=String(fields.naam||'').trim(); if(!id&&T.medewerkers.some(m=>m.naam.toLowerCase()===naam.toLowerCase())){ const m=T.medewerkers.find(m=>m.naam.toLowerCase()===naam.toLowerCase()); m.actief=true; log(); return [200,{ok:true,id:m.id}]; }
        if(id){ if(!admin){ const toegestaan={}; if('naam' in fields) toegestaan.naam=naam; Object.assign(T.medewerkers.find(m=>m.id===id),toegestaan); } else Object.assign(T.medewerkers.find(m=>m.id===id),fields); log(); return [200,{ok:true,id}]; }
        const m=add('medewerkers','medewerker',{functie:'',actief:true,sort:100,...fields,naam}); log(); return [200,{ok:true,id:m.id}]; }
...
function seedWeek35(){
  ...
  DAGEN.forEach((d,i)=>{
    ...
    const shifts=d.arbeid.map(([n,u],j)=>({start:'06:00',eind:tijd(6*60+Math.round((u+0.5)*60)),pauze:'',namen:(i===0&&j===0)?NAMEN.slice(0,Math.min(n,3)):[],extra_man:(i===0&&j===0)?n-Math.min(n,3):n,overnachting:true,nachten:''}));
