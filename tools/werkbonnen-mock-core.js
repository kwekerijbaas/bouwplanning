...
function normRegels(regels){ if(!Array.isArray(regels)) throw new Error('regels ontbreken'); return regels.map((x,i)=>{ if(!CATS.includes(x.categorie)) throw new Error('onbekende categorie op regel '+(i+1)); const oms=String(x.omschrijving||'').trim(); if(!oms) throw new Error('omschrijving leeg op regel '+(i+1)); const aantal=num(x.aantal,'aantal'); const per=x.per==null||x.per===''?null:num(x.per,'per'); const prijs=num(x.prijs,'prijs'); if(x.categorie!=='transport'&&!Number.isInteger(aantal)) throw new Error('aantal op regel '+(i+1)+' moet een heel getal zijn (geen halve mannen, auto\'s, nachten of stuks)'); if(per!=null&&String(x.eenheid_per||'')!=='uur'&&!Number.isInteger(per)) throw new Error('km/dagen op regel '+(i+1)+' moeten een heel getal zijn'); const totaal=r2(per==null?aantal:aantal*per); return {sort:i+1,categorie:x.categorie,omschrijving:oms,tarief_id:x.tarief_id==null||x.tarief_id===''?null:Number(x.tarief_id),aantal,per,totaal,prijs,bedrag:r2(totaal*prijs),eenheid_n:String(x.eenheid_n||''),eenheid_per:String(x.eenheid_per||''),eenheid_totaal:String(x.eenheid_totaal||''),namen:String(x.namen||'')}; }); }
