...
[['arbeid','Arbeid','man','uur','per uur',61.50,'ma-vr',1],['arbeid','Arbeid zaterdag','man','uur','per uur',77.00,'za',2],['arbeid','Arbeid zondag / feestdag','man','uur','per uur',92.25,'zo',3],
 ...
 // aanvulling uit KZ-facturen 2026200 (wk 26+27) en 2026221/2026222 (wk 30)
 ['materieel','Gebruik trekker met kipper incl. brandstof','','','per dag',200,'alle',38],['materieel','Gebruik gootkarren A-type','stuks','dag','per dag',80,'alle',39],
 ['transport','Transport container','','','per uur',95,'alle',40],['transport','Transport materiaal','','','per uur',95,'alle',41],['transport','Ophalen materiaal','','','per uur',130,'alle',42],['transport','Transport materieel (gebracht)','','','per km',1.50,'alle',43],['transport','Transport machines','','','per uur',115,'alle',44],
 ...
 ['materiaal','Gootstrip','','','per stuk',4.38,'alle',57],['materiaal','Nokbeveiliging','','','per stuk',6.25,'alle',58],['materiaal','Dekroede','','','per stuk',15.65,'alle',59],['materiaal','Kaproede','','','per stuk',18.75,'alle',60],
 ['materiaal','Goot-nok-goot kabel recht','','','per stuk',28.13,'alle',61],['materiaal','Goot-nok-goot kabel schuin','','','per stuk',28.13,'alle',62],['materiaal','Nok koppeling','','','per stuk',6.25,'alle',63],['materiaal','Nokprofiel','','','per stuk',6.25,'alle',64],
 ['materiaal','Nokblok + parkers','','','per stuk',9.38,'alle',65],['materiaal','Nokblok inclusief parkers','','','per stuk',4.38,'alle',66],['materiaal','Parkers','','','per stuk',0.32,'alle',67],['materiaal','Gootlepels + ring + parkers','','','per stuk',3.13,'alle',68],
 ['materiaal','Luchtramen compleet','','','per stuk',143.25,'alle',69],['materiaal','Betonplex','','','per stuk',6.00,'alle',70]
...
// KZ rekent zoals hun Excel: regelbedragen (totaal x prijs) onafgerond optellen, pas het eindtotaal op centen afronden.
function somExact(regels){ let c=0; for(const r of regels) c+=Math.round(Number(r.totaal)*100)*Math.round(Number(r.prijs)*100); return Math.sign(c)*Math.floor((Math.abs(c)+50)/100)/100; }
...
// Na een prijswijziging volgen open (niet-gefactureerde) bonnen de nieuwe prijs (= wb_herprijs in de database).
function herprijs(projectId){ for(const b of T.bonnen){ if(b.status==='gefactureerd'||(projectId!=null&&b.project_id!==projectId)) continue; prijsVanServer(b.project_id,T.regels.filter(r=>r.bon_id===b.id&&r.tarief_id!=null)); } }
  ...
  try{
    switch(body.action){
      ...
      case 'bon_save': {
        ...
        const regels=normRegels(body.regels); prijsVanServer(Number(fields.project_id),regels); const invoer=normInvoer(fields.invoer);
      ...
      case 'bon_beoordeel': { if(!admin) throw new Error('alleen administratie'); const bon=T.bonnen.find(b=>b.id===id); if(bon.status==='gefactureerd') throw new Error('bon zit al op een factuur'); const b=body.besluit; if(!['goedgekeurd','afgekeurd','ingediend'].includes(b)) throw new Error('onbekend besluit'); const los=b==='goedgekeurd'&&T.regels.find(r=>r.bon_id===bon.id&&r.tarief_id==null); if(los) throw new Error("regel '"+los.omschrijving+"' heeft nog geen type: kies een type of maak een nieuw type met prijs aan"); Object.assign(bon,{status:b,beoordeeld_door:b==='ingediend'?'':wie,beoordeeld_ts:b==='ingediend'?null:new Date().toISOString(),beoordeling:String(body.beoordeling||'')}); break; }
      ...
      case 'projecttarief_save': { // projectafspraken: prijs per tarief voor dit project (wizard)
        ...
        for(const x of prijzen){ const tarief_id=Number(x.tarief_id); if(!T.tarieven.some(t=>t.id===tarief_id)) throw new Error('tarief '+x.tarief_id+' niet gevonden'); if(x.prijs==null||x.prijs===''){ T.projecttarieven=T.projecttarieven.filter(y=>!(y.project_id===project_id&&y.tarief_id===tarief_id)); continue; } const prijs=num(x.prijs,'prijs'); const b=T.projecttarieven.find(y=>y.project_id===project_id&&y.tarief_id===tarief_id); if(b) b.prijs=prijs; else add('projecttarieven','projecttarief',{project_id,tarief_id,prijs}); }
        herprijs(project_id); break; }
      ...
      case 'tarief_import': { // export uit het boekhoudpakket (CSV: omschrijving;prijs;eenheid) -> tarieven (materiaal/materieel/transport)
        ...
        if(bijgewerkt) herprijs(null); log(); return [200,{ok:true,nieuw,bijgewerkt}]; }
      case 'tarief_save': { if(!admin) throw new Error('alleen administratie'); if('categorie' in fields&&!CATS.includes(fields.categorie)) throw new Error('onbekende categorie'); if('dagtype' in fields&&!['alle','ma-vr','za','zo'].includes(fields.dagtype)) throw new Error('onbekend dagtype'); if('prijs' in fields) fields.prijs=num(fields.prijs,'prijs'); if('omschrijving' in fields&&T.tarieven.some(t=>t.id!==id&&t.categorie===(fields.categorie||(T.tarieven.find(x=>x.id===id)||{}).categorie)&&t.omschrijving.toLowerCase()===String(fields.omschrijving).toLowerCase())) throw new Error('dit type bestaat al'); let t; if(id){ t=T.tarieven.find(b=>b.id===id); Object.assign(t,fields); if('prijs' in fields) herprijs(null); } else t=add('tarieven','tarief',{actief:true,eenheid_n:'',eenheid_per:'',eenheid_totaal:'',dagtype:'alle',sort:100,...fields}); log(); return [200,{ok:true,id:t.id}]; }
      ...
      case 'factuur_maak': {
        if(!admin) throw new Error('alleen administratie'); const project_id=Number(body.project_id), jaar=Number(body.jaar), week=Number(body.week); const weekTot=body.week_tot==null||body.week_tot===''?week:Number(body.week_tot); if(!Number.isInteger(weekTot)||weekTot<week||weekTot>53) throw new Error('t/m week moet gelijk aan of na week '+week+' liggen'); const ids=body.bon_ids||[]; if(!ids.length) throw new Error('geen goedgekeurde bonnen geselecteerd');
        const bonnen=T.bonnen.filter(b=>ids.includes(b.id)); for(const b of bonnen){ if(b.project_id!==project_id) throw new Error('bon '+b.id+' hoort bij een ander project'); if(b.status!=='goedgekeurd') throw new Error('bon van '+b.datum+' is nog niet goedgekeurd'); const w=isoWeek(b.datum); if(w.jaar!==jaar||w.week<week||w.week>weekTot) throw new Error('bon van '+b.datum+' valt niet in week '+(weekTot===week?week:week+' t/m '+weekTot)); }
        const excl=somExact(T.regels.filter(r=>ids.includes(r.bon_id))); const pct=Number(T.instellingen.btw_pct||21); const btw=r2(excl*pct/100);
